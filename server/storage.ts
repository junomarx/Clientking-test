...
export class DatabaseStorage implements IStorage {
  ...
  async deleteUser(id: number, currentUserId?: number): Promise<boolean> {
    ...
    try {
      console.log(
        `Beginne mit dem Löschen des Benutzers mit ID ${id} und aller zugehörigen Daten...`,
      );
      ...
      try {
        ...
        for (const customer of userCustomers) {
          try {
            ...
            await db.delete(repairs).where(eq(repairs.customerId, customer.id));

            console.log(`Reparaturen für Kunde ${customer.id} gelöscht`);
          } catch (err) {
            const error = err as Error;
            console.warn(
              `Fehler beim Löschen der Daten für Kunde ${customer.id}:`,
              error.message,
            );
        ...
        try {
          await db.delete(repairs).where(eq(repairs.userId, id));

          console.log(`Alle direkten Reparaturen des Benutzers gelöscht`);
        } catch (err) {
          const error = err as Error;
          console.warn(
            `Fehler beim Löschen der Reparaturen des Benutzers:`,
            error.message,
          );
        }
        ...
        try {
          await db.delete(costEstimates).where(eq(costEstimates.userId, id));

          console.log(
            `Alle direkten Kostenvoranschläge des Benutzers gelöscht`,
          );
        } catch (err) {
          const error = err as Error;
          console.warn(
            `Fehler beim Löschen der Kostenvoranschläge des Benutzers:`,
            error.message,
          );
        }
      ...
      try {
        await db.delete(emailTemplates).where(eq(emailTemplates.userId, id));
      ...
      try {
        await db.delete(userModels).where(eq(userModels.userId, id));
      ...
      try {
        await db.delete(userModelSeries).where(eq(userModelSeries.userId, id));
      ...
      try {
        await db.delete(userBrands).where(eq(userBrands.userId, id));

        console.log(`Alle benutzerdefinierten Marken/Hersteller gelöscht`);
      } catch (err) {
        const error = err as Error;
        console.warn(
          `Fehler beim Löschen der Marken/Hersteller:`,
          error.message,
        );
      }
      ...
      try {
        await db.delete(userDeviceTypes).where(eq(userDeviceTypes.userId, id));
      ...
      try {
        ...
        const error = err as Error;
        console.warn(
          `Fehler beim Löschen der Business-Einstellungen:`,
          error.message,
        );
      }
      ...
      try {
        ...
        await db.execute(
          sql`DELETE FROM sessions WHERE sess->'user'->>'id' = ${id.toString()}`,
        );
      ...
      try {
        ...
        const error = err as Error;
        console.error(
          `KRITISCHER FEHLER: Benutzer konnte nicht gelöscht werden:`,
          error.message,
        );
        return false;
      }
      ...
      console.log(
        `Benutzer mit ID ${id} und alle zugehörigen Daten wurden erfolgreich gelöscht.`,
      );
      return true;
    } catch (error) {
  ...
  async getAllCustomers(currentUserId?: number): Promise<Customer[]> {
    ...
    console.log(
      `getAllCustomers: Benutzer ${currentUser.username} (ID: ${currentUserId}) mit Shop-ID ${shopIdValue} - isAdmin: ${currentUser.isAdmin}`,
    );
  ...
  async getAllRepairs(currentUserId?: number): Promise<Repair[]> {
    ...
    // Debug-Ausgabe für Shop-Isolation
    console.log(
      `getAllRepairs: Benutzer ${currentUser.username} (ID: ${currentUserId}) mit Shop-ID ${shopIdValue} - isAdmin: ${currentUser.isAdmin}`,
    );
  ...
  async canCreateNewRepair(userId: number): Promise<{
    canCreate: boolean;
    currentCount: number;
    limit: number;
    pricingPlan: string;
  ...
  async createRepair(
    ...
    if (currentUserId) {
      ...
      if (!canCreate.canCreate) {
        throw new Error(
          `Monatliches Limit erreicht: ${canCreate.currentCount}/${canCreate.limit} Reparaturen im Basic-Paket`,
        );
      }
    }
    ...
    const [repair] = await db.insert(repairs).values(repairData).returning();
  ...
  async updateRepair(
    id: number,
    repairUpdate: Partial<InsertRepair>,
    currentUserId?: number,
  ): Promise<Repair | undefined> {
    if (!currentUserId) {
      return undefined; // Wenn keine Benutzer-ID angegeben ist, gebe undefined zurück
  ...
  async updateRepairSignature(
    id: number,
    signature: string,
    type: "dropoff" | "pickup" = "dropoff",
    currentUserId?: number,
  ): Promise<Repair | undefined> {
    // DSGVO-konform: Ohne Benutzer-ID kein Zugriff auf Daten
    if (!currentUserId) {
      console.log(
        `Sicherheitswarnung: updateRepairSignature ohne Benutzer-ID aufgerufen für Reparatur ${id}`,
      );
      return undefined;
    }
    ...
    if (!currentUser) {
      console.log(
        `Sicherheitswarnung: Benutzer ${currentUserId} nicht gefunden beim Aktualisieren der Signatur für Reparatur ${id}`,
      );
      return undefined;
    }
  ...
  async getBusinessSettings(
    ...
    try {
      ...
      if (!userId) {
        console.log(
          "Keine Benutzer-ID angegeben, kann keine Geschäftseinstellungen abrufen",
        );
        return undefined;
      }
      ...
      console.log(
        `Suche nach Geschäftseinstellungen für Benutzer ${user.username} (ID ${userId}, Shop ${shopId})`,
      );
      ...
      if (userSettings) {
        console.log(
          `Gefunden: Einstellungen mit ID ${userSettings.id} für User ${userSettings.userId} (Shop ${shopId})`,
        );
        return userSettings;
      }
      ...
      if (shopSettings) {
        console.log(
          `Shop-Einstellungen gefunden: ID ${shopSettings.id} für Shop ${shopId}`,
        );
        return shopSettings;
      }

      console.log(
        `Keine Einstellungen für Benutzer ${user.username} (ID ${userId}, Shop ${shopId}) gefunden`,
      );
      return undefined;
    } catch (error) {
  ...
  async updateBusinessSettings(
    settingsData: Partial<InsertBusinessSettings>,
    userId?: number,
  ): Promise<BusinessSettings> {
    try {
      // Debug-Informationen
      ...
      if (!userId) {
        ...
        throw new Error(
          "User ID ist erforderlich, um Geschäftseinstellungen zu aktualisieren",
        );
      }
      ...
      if (!user) {
        console.error(
          `⭐⭐⭐ KRITISCHER FEHLER - Benutzer mit ID ${userId} nicht gefunden!`,
        );
      ...
      console.log(
        "⭐ Suche nach Einstellungen mit Shop-Isolation für Benutzer",
        user.username,
      );
      ...
      if (existingSettings) {
        console.log(
          `⭐ Updating settings with ID ${existingSettings.id} for user ${userId} (Shop ${shopId})`,
        );
        ...
        console.log(
          ...
          "in shop",
          updatedSettings.shopId,
        );
        return updatedSettings;
      } else {
        // Create new settings for this user
        console.log(
          "Creating new business settings for user",
          userId,
          "in shop",
          shopId,
        );
        ...
        if (!dataToInsert.userId) {
          console.warn(
            "User ID is undefined in insert data, forcing to user ID:",
            userId,
          );
        ...
        if (!dataToInsert.shopId) {
          console.warn(
            "Shop ID is undefined in insert data, forcing to shop ID:",
            shopId,
          );
  ...
  async getStats(
    ...
    // Debug-Ausgabe für Shop-Isolation
    console.log(
      `getStats: Benutzer ${currentUser.username} (ID: ${currentUserId}) mit Shop-ID ${shopIdValue} - isAdmin: ${currentUser.isAdmin}`,
    );
    ...
    const [inRepairResult] = await db
      .select({ count: count() })
      .from(repairs)
      .where(and(eq(repairs.status, "in_reparatur"), combinedFilter));
    ...
    const [completedResult] = await db
      ...
      .where(
        and(
          or(eq(repairs.status, "fertig"), eq(repairs.status, "abgeholt")),
          combinedFilter,
        ),
      );
  ...
  async getDetailedRepairStats(
    ...
    try {
      ...
      // Debug-Ausgabe für Shop-Isolation
      console.log(
        `getDetailedRepairStats: Benutzer ${currentUser.username} (ID: ${currentUserId}) mit Shop-ID ${shopIdValue} - isAdmin: ${currentUser.isAdmin}`,
      );
      ...
      const userRepairs = await db.select().from(repairs).where(combinedFilter);
  ...
  async sendEmailWithTemplate(
    ...
    try {
      console.log(
        "Storage sendEmailWithTemplate aufgerufen mit templateId:",
        templateId,
      );
  ...
  async deleteUserDeviceType(id: number, userId: number): Promise<boolean> {
    try {
      ...
      if (relatedBrands.length > 0) {
        console.error(
          `Gerätetyp mit ID ${id} kann nicht gelöscht werden, da er von ${relatedBrands.length} Marken verwendet wird.`,
        );
        return false;
      }
  ...
  async getUserBrand(
    ...
    const [brand] = await db
      .select()
      .from(userBrands)
      .where(and(eq(userBrands.id, id), eq(userBrands.userId, userId)));
    return brand;
  }

  async getUserBrandsByDeviceTypeId(
    deviceTypeId: number,
    userId: number,
  ): Promise<UserBrand[]> {
    // Benutzer abrufen, um Shop-ID zu erhalten
  ...
  async getUserModelSeriesByBrandId(
    brandId: number,
    userId: number,
  ): Promise<UserModelSeries[]> {
    return await db
  ...
  async getUserModelSeriesByNameAndBrand(
    name: string,
    brandId: number,
    userId: number,
  ): Promise<UserModelSeries | undefined> {
  ...
  async getUserModelSeries_ByDeviceTypeAndBrand(
    deviceTypeId: number,
    brandId: number,
    userId: number,
  ): Promise<UserModelSeries[]> {
  ...
  async createUserModelSeries(
    ...
    if (existingModelSeries.length > 0) {
      console.log(
        `[Modellreihe] Eine Modellreihe mit dem Namen '${modelSeries.name}' existiert bereits für Brand ${modelSeries.brandId}`,
      );
    ...
    console.log(
      `[Modellreihe] Erstelle neue Modellreihe '${modelSeries.name}' für Brand ${modelSeries.brandId}`,
    );
  ...
  async updateUserModelSeries(
    id: number,
    modelSeriesData: Partial<InsertUserModelSeries>,
    userId: number,
  ): Promise<UserModelSeries | undefined> {
    const [updatedModelSeries] = await db
      ...
      .where(
        and(eq(userModelSeries.id, id), eq(userModelSeries.userId, userId)),
      )
      .returning();
  ...
  async deleteAllUserModelSeriesForBrand(
    brandId: number,
    userId: number,
  ): Promise<boolean> {
    try {
  ...
  async updateUserModel(
    id: number,
    modelData: Partial<InsertUserModel>,
    userId: number,
  ): Promise<UserModel | undefined> {
  ...
  async deleteAllUserModelsForModelSeries(
    modelSeriesId: number,
    userId: number,
  ): Promise<boolean> {
    try {
  ...
  async getCostEstimate(
    id: number,
    currentUserId?: number,
  ): Promise<(CostEstimate & { customer?: any }) | undefined> {
    if (!currentUserId) {
      return undefined; // Wenn keine Benutzer-ID angegeben ist, gebe undefined zurück
    }
  ...
  async getCostEstimatesByCustomerId(
    customerId: number,
    currentUserId?: number,
  ): Promise<CostEstimate[]> {
    if (!currentUserId) {
  ...
  async createCostEstimate(
    ...
    if (!currentUserId) {
      throw new Error(
        "Benutzer-ID erforderlich, um einen Kostenvoranschlag zu erstellen",
      );
    }
    ...
    const itemPrices = items.map((item) =>
      this.extractNumberFromString(item.totalPrice),
    );
  ...
  async updateCostEstimate(
    ...
    if (estimateUpdate.items) {
      const items = Array.isArray(estimateUpdate.items)
        ? estimateUpdate.items
        : [];
      ...
      // Berechne die MwSt
      const taxRateValue = this.extractNumberFromString(
        estimateUpdate.taxRate || currentEstimate.taxRate,
      );
      const taxAmountValue = subtotalValue * (taxRateValue / 100);
      ...
      const subtotalValue = this.extractNumberFromString(
  ...
  async convertToRepair(
    ...
    const repairData: InsertRepair = {
      ...
      status: "eingegangen",
      notes:
        `Erstellt aus Kostenvoranschlag ${estimate.referenceNumber}. ${estimate.notes || ""}`.trim(),
    };
