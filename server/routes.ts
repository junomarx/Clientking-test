...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/check-feature-access/:feature", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      // Direkt die hasAccess-Funktion verwenden, um zu prüfen, ob der Benutzer Zugriff hat
      ...
      res.json({
        ...
        pricingPlan: user.pricingPlan,
        featureOverridesPresent: !!user.featureOverrides,
        username: user.username
      });
    } catch (error) {
      console.error("Fehler beim Prüfen des Feature-Zugriffs:", error);
      res.status(500).json({ message: "Fehler beim Prüfen des Feature-Zugriffs" });
    }
  });
  
  // Teste, ob der Benutzer mindestens Professional-Ebene hat (für bestimmte Features)
  app.get("/api/check-professional", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const isProfessional = await isProfessionalOrHigher(userId);
      const user = req.user as any;
      ...
      res.json({
        isProfessionalOrHigher: isProfessional,
        pricingPlan: user.pricingPlan,
        featureOverridesPresent: !!user.featureOverrides,
        username: user.username
      });
    } catch (error) {
      console.error("Fehler beim Prüfen des Professional-Zugriffs:", error);
      res.status(500).json({ message: "Fehler beim Prüfen des Professional-Zugriffs" });
    }
  });
  
  // Teste, ob der Benutzer die Enterprise-Ebene hat
  app.get("/api/check-enterprise", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any).id;
      const isEnterpriseUser = await isEnterprise(userId);
      const user = req.user as any;
      ...
      console.error("Fehler beim Prüfen des Enterprise-Zugriffs:", error);
      res.status(500).json({ message: "Fehler beim Prüfen des Enterprise-Zugriffs" });
    }
  });
  
  // Feature-Übersteuerungen für aktuellen Benutzer anzeigen
  app.get("/api/feature-overrides", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const user = req.user as any;
      ...
      if (user.featureOverrides && typeof user.featureOverrides === 'string') {
        try {
          ...
          console.warn(`Fehler beim Parsen der Feature-Übersteuerungen für ${user.username}:`, e);
        }
      }
      ...
      console.error("Fehler beim Abrufen der Feature-Übersteuerungen:", error);
      res.status(500).json({ message: "Fehler beim Abrufen der Feature-Übersteuerungen" });
    }
  });
  
  // Feature-Übersteuerungen für aktuellen Benutzer aktualisieren (nur für Test-/Debug-Zwecke)
  app.post("/api/feature-overrides", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const user = req.user as any;
      const overrides = req.body;
      
      // Nur für Admin oder den Benutzer selbst erlauben
      if (!user.isAdmin && user.username !== 'bugi') {
        return res.status(403).json({ message: "Keine Berechtigung zum Ändern von Feature-Übersteuerungen" });
      }
      
      // Überprüfen, ob die Übersteuerungen ein gültiges Format haben
      if (typeof overrides !== 'object') {
        return res.status(400).json({ message: "Ungültiges Format für Feature-Übersteuerungen" });
      }
      
      // Übersteuerungen als JSON-String speichern
      const result = await db.update(users)
        .set({ featureOverrides: JSON.stringify(overrides) })
      ...
      console.error("Fehler beim Aktualisieren der Feature-Übersteuerungen:", error);
      res.status(500).json({ message: "Fehler beim Aktualisieren der Feature-Übersteuerungen" });
    }
  });
