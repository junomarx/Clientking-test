...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/repairs/:id/quote", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const repairId = parseInt(req.params.id);
      ...
      // Quote Response in der Datenbank erstellen
      const [quoteResponse] = await db.insert(quoteResponses).values({
        repairId: repairId,
        token: token,
      ...
      // Geschätzten Preis in der Reparatur aktualisieren
      await db.update(repairs)
        .set({ estimatedCost: quotedAmount })
        .where(eq(repairs.id, repairId));
      ...
      if (sendEmail) {
        ...
        try {
          ...
          // Quote response URLs generieren
      ...
      res.json({ 
        success: true, 
        message: sendEmail ? "Kostenvoranschlag wurde gesendet" : "Kostenvoranschlag wurde erstellt",
  ...
  // Kunde-Antwort auf Kostenvoranschlag anzeigen
  app.get("/quote-response/:token", async (req: Request, res: Response) => {
    try {
      const { token } = req.params;
      const { action } = req.query;

      // Quote Response finden
      const [quoteResponse] = await db.select()
        .from(quoteResponses)
        .where(eq(quoteResponses.token, token));

      if (!quoteResponse) {
        return res.status(404).send(`
          <html><body>
            <h1>Kostenvoranschlag nicht gefunden</h1>
            <p>Der angeforderte Kostenvoranschlag existiert nicht oder ist abgelaufen.</p>
          </body></html>
        `);
      }

      // Reparatur-Details laden
      const [repair] = await db.select()
        .from(repairs)
        .where(eq(repairs.id, quoteResponse.repairId));

      if (!repair) {
        return res.status(404).send(`
          <html><body>
            <h1>Reparatur nicht gefunden</h1>
          </body></html>
        `);
      }
      ...
      if (quoteResponse.response) {
        const statusText = quoteResponse.response === 'accepted' ? 'akzeptiert' : 'abgelehnt';
        return res.send(`
          <html><body style="font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px;">
            <h1>Bereits beantwortet</h1>
            <p>Sie haben diesen Kostenvoranschlag bereits <strong>${statusText}</strong>.</p>
            <p>Vielen Dank für Ihre Antwort!</p>
          </body></html>
        `);
      }

      // Aktions-spezifische Anzeige
      if (action === 'accept' || action === 'decline') {
        ...
        return res.send(`
          <html><body style="font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px;">
            <h1>Kostenvoranschlag ${actionText}</h1>
            <div style="background-color: #f3f4f6; padding: 20px; border-radius: 10px; margin: 20px 0;">
              <h3>Reparatur-Details:</h3>
              <p><strong>Gerät:</strong> ${repair.brand} ${repair.model}</p>
              <p><strong>Geschätzter Preis:</strong> ${quoteResponse.quotedAmount} €</p>
              ${quoteResponse.quoteDescription ? `<p><strong>Beschreibung:</strong> ${quoteResponse.quoteDescription}</p>` : ''}
            </div>
            <form method="POST" action="/quote-response/${token}">
              <input type="hidden" name="action" value="${action}">
              ${action === 'decline' ? `
                <div style="margin: 20px 0;">
                  <label for="reason" style="display: block; margin-bottom: 10px;">Grund für die Ablehnung (optional):</label>
                  <textarea name="reason" id="reason" rows="3" style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px;"></textarea>
                </div>
              ` : ''}
              <button type="submit" style="background-color: ${actionColor}; color: white; padding: 15px 30px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px;">
                Kostenvoranschlag ${actionText}
              </button>
            </form>
          </body></html>
        `);
      ...
      return res.send(`
        <html><body style="font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px;">
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="/quote-response/${token}?action=accept" style="background-color: #22c55e; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; margin: 10px;">✅ Preis akzeptieren</a>
            <a href="/quote-response/${token}?action=decline" style="background-color: #ef4444; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; margin: 10px;">❌ Preis ablehnen</a>
          </div>
        </html></body>
      `);

    } catch (error) {
      console.error("Fehler beim Anzeigen der Quote-Response:", error);
      res.status(500).send(`
        <html><body>
          <h1>Fehler</h1>
  ...
  // Kunde-Antwort verarbeiten
  app.post("/quote-response/:token", async (req: Request, res: Response) => {
    try {
      const { token } = req.params;
      ...
      if (quoteResponse.response) {
        return res.status(400).send(`
          <html><body>
            <h1>Bereits beantwortet</h1>
            <p>Dieser Kostenvoranschlag wurde bereits beantwortet.</p>
          </body></html>
        `);
      }

      // Antwort speichern
      await db.update(quoteResponses)
        .set({
          response: action,
      ...
      // TODO: WebSocket-Benachrichtigung an Shop senden
      // const onlineStatusManager = getOnlineStatusManager();
      // onlineStatusManager.notifyQuoteResponse(quoteResponse.repairId, action);

      const actionText = action === 'accept' ? 'akzeptiert' : 'abgelehnt';
      
      res.send(`
        <html><body style="font-family: Arial, sans-serif; max-width: 600px; margin: 50px auto; padding: 20px; text-align: center;">
          <h1>Vielen Dank!</h1>
          <p>Sie haben den Kostenvoranschlag <strong>${actionText}</strong>.</p>
          <p>Wir werden uns zeitnah bei Ihnen melden.</p>
          <div style="background-color: #f3f4f6; padding: 20px; border-radius: 10px; margin: 20px 0;">
            <p><strong>Ihre Antwort wurde erfolgreich übermittelt.</strong></p>
          </div>
        </body></html>
      `);

    } catch (error) {
      console.error("Fehler beim Verarbeiten der Quote-Response:", error);
      res.status(500).send(`
        <html><body>
          <h1>Fehler</h1>
          <p>Ein Fehler ist aufgetreten. Bitte versuchen Sie es später erneut.</p>
