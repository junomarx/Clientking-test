...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/orders/accessories/bulk-update", async (req: Request, res: Response) => {
    try {
      ...
      if (success) {
        // Automatische E-Mail-Versendung bei Status "eingetroffen"
        if (status === "eingetroffen") {
          console.log(`🚀 AUTOMATISCHER E-MAIL-VERSAND: Sende Ankunfts-E-Mails für ${accessoryIds.length} Zubehör-Artikel`);
          ...
          // Für jedes Zubehör eine E-Mail senden
          for (const accessoryId of accessoryIds) {
            try {
              // Zubehör-Daten abrufen
              const accessory = await storage.getAccessory(accessoryId, userId);
              if (!accessory || !accessory.customerId) {
                console.log(`⏩ Überspringe Zubehör ${accessoryId}: Kein Kunde zugeordnet (Lager-Artikel)`);
                continue;
              }
              
              // Bereits gesendet prüfen
              if (accessory.emailSent) {
                console.log(`⏩ Überspringe Zubehör ${accessoryId}: E-Mail bereits gesendet`);
                continue;
              }
              ...
              const customer = await storage.getCustomer(accessory.customerId, userId);
              if (!customer || !customer.email) {
                console.log(`⚠️ Überspringe Zubehör ${accessoryId}: Kunde nicht gefunden oder keine E-Mail`);
                continue;
              }
              ...
              if (emailResult) {
                // E-Mail-Status in der Datenbank aktualisieren
                await storage.updateAccessory(accessoryId, { emailSent: true }, userId);
                console.log(`✅ E-Mail erfolgreich gesendet für Zubehör ${accessoryId} an ${customer.email}`);
              } else {
                console.error(`❌ E-Mail-Versand fehlgeschlagen für Zubehör ${accessoryId}`);
              }
              
            } catch (emailError) {
              console.error(`❌ Fehler beim E-Mail-Versand für Zubehör ${accessoryId}:`, emailError);
            }
          }
        }
