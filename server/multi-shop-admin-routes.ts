...
export function registerMultiShopAdminRoutes(app: Express) {
  ...
  // MSA Reparatur-Status aktualisieren
  app.patch("/api/multi-shop/repairs/:id/status", protectMultiShopAdmin, async (req, res) => {
    try {
      ...
      if (!repairId || isNaN(repairId)) {
        return res.status(400).json({ error: "Ungültige Reparatur-ID" });
      }
      ...
      // Reparatur laden und prüfen ob Zugriff erlaubt
      ...
      const authorizedShopIds = authorizedShops.map(s => s.shopId);
      if (!authorizedShopIds.includes(repair.shopId)) {
        return res.status(403).json({ error: "Keine Berechtigung für diesen Shop" });
      }
      ...
      if (oldStatus !== status) {
        ...
        console.log(`📋 MSA Activity-Log erstellt: Reparatur ${updatedRepair.orderCode} Status ${oldStatus} → ${status} von ${currentUser.username}`);
      }
      ...
      console.error("MSA Reparatur-Status-Update Fehler:", error);
      res.status(500).json({ error: "Fehler beim Aktualisieren des Reparatur-Status" });
    }
  });
