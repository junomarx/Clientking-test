...
export default function EditCostEstimateForm({ estimateId, onSuccess }: EditCostEstimateFormProps) {
  ...
  // Query für den Kostenvoranschlag
  const { data: estimate, isLoading: isLoadingEstimate, isError } = useQuery({
  ...
  // Daten in Formular setzen, sobald der Kostenvoranschlag geladen ist
  useEffect(() => {
    if (estimate) {
      try {
        // Datumskonvertierung
        ...
        toast({
          title: "Fehler",
          description: "Die Daten konnten nicht in das Formular geladen werden.",
          variant: "destructive",
        });
      }
    }
  ...
  // Mutation zum Aktualisieren eines Kostenvoranschlags
  const updateMutation = useMutation({
    mutationFn: async (data: FormValues) => {
      const response = await apiRequest("PATCH", `/api/cost-estimates/${estimateId}`, data);
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Fehler beim Aktualisieren des Kostenvoranschlags");
      }
      return response.json();
    },
    onSuccess: () => {
      toast({
        title: "Erfolg",
        description: "Kostenvoranschlag wurde erfolgreich aktualisiert"
      });
      // Cache invalidieren
  ...
  if (isLoading || isLoadingEstimate) {
    return (
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select 
                ...
                disabled={estimate.status !== 'offen'}
              >
                <FormControl>
                  <SelectTrigger>
              ...
              <div key={field.id} className="grid grid-cols-12 gap-2 mb-3">
                ...
                <div className="col-span-2 flex items-end mb-1">
                  <Button 
                    ...
                    disabled={fields.length === 1 || estimate.status !== 'offen'}
                  >
        ...
        <div className="flex justify-end">
          <Button 
            type="submit" 
            disabled={updateMutation.isPending || estimate.status !== 'offen'}
          >
            {updateMutation.isPending ? "Wird aktualisiert..." : "Kostenvoranschlag aktualisieren"}
          </Button>
        </div>
      </form>
    </Form>
