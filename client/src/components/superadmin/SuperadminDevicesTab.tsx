...
// CRUD-Funktionen für den Fehlerkatalog
function handleSubmitCreateIssue() {
  const { toast } = useToast();
  const createIssueMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", "/api/admin/device-issues", issueForm);
      if (!response.ok) {
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Der Fehlereintrag wurde erfolgreich erstellt.",
      });
    },
    onError: (error: Error) => {
      toast({
...
function handleSubmitEditIssue() {
  ...
  const updateIssueMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest(
        "PATCH",
        `/api/admin/device-issues/${selectedIssue.id}`,
        issueForm
      );
      if (!response.ok) {
        const errorData = await response.json();
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Der Fehlereintrag wurde erfolgreich aktualisiert.",
      });
    },
    onError: (error: Error) => {
      toast({
...
function handleDeleteIssue(id: number) {
  const { toast } = useToast();
  const deleteIssueMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Der Fehlereintrag wurde erfolgreich gelöscht.",
      });
    },
    onError: (error: Error) => {
      toast({
