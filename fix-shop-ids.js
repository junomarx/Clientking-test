/**
 * Skript zur Korrektur von duplizierten Shop-IDs
 */

// Da es Schwierigkeiten mit der direkten Ausführung des Migration-Skripts gibt,
// erstellen wir eine SQL-Datei, die direkt auf die Datenbank angewendet werden kann

const fs = require('fs');

// SQL Abfrage, um duplicierte Shop-IDs zu finden
...
// SQL Abfrage, um die höchste Shop-ID zu finden
const findMaxShopIdSql = `
SELECT COALESCE(MAX(shop_id), 0) as max_shop_id FROM users WHERE shop_id IS NOT NULL;
`;

// SQL, um Unique Constraint hinzuzufügen
...
// Generiere komplettes SQL-Skript
const completeSql = `
...
// Schreibe SQL in Datei
fs.writeFileSync('fix-duplicate-shop-ids.sql', completeSql);

console.log('SQL-Skript zur Korrektur duplizierter Shop-IDs wurde erzeugt.');
console.log('Führe das SQL-Skript mit dem folgenden Befehl aus:');
console.log('psql $DATABASE_URL -f fix-duplicate-shop-ids.sql');